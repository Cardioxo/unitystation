...
public class DamageOnPickUp : MonoBehaviour, IServerInventoryMove
{
	...
	private void DoDamage(InventoryMove info)
	{
		if (doesDamage && Random.value < doesDamageChance)
		{
			...
			{
				player.OrganicHealthSystem.ApplyDamageToBodypart(gameObject, amountOfDamage, attackType, damageType, BodyPartType.LeftArm);
			}
			else
			{
				player.OrganicHealthSystem.ApplyDamageToBodypart(gameObject, amountOfDamage, attackType, damageType, BodyPartType.RightArm);
			}
